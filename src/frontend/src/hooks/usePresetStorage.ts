...
export function usePresetStorage() {
  const [presets, setPresets] = useState<Record<string, StudioPreset>>(() =>
    loadFromStorage(),
  );

  const savePreset = useCallback(
    (name: string, preset: Omit<StudioPreset, "name" | "createdAt">) => {
  ...
  const listPresets = useCallback((): string[] => {
    return Object.keys(presets).sort(
      (a, b) => (presets[b].createdAt ?? 0) - (presets[a].createdAt ?? 0),
    );
  }, [presets]);
