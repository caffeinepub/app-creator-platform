...
import { type AudioTuningParams, DEFAULT_PARAMS } from "../data/audioPresets";
import type { SynthParams } from "../data/soundLibrary";
...
function createReverbIR(ctx: AudioContext, roomSize: number): AudioBuffer {
  ...
  for (let ch = 0; ch < 2; ch++) {
    ...
    for (let i = 0; i < frameCount; i++) {
      data[i] = (Math.random() * 2 - 1) * (1 - i / frameCount) ** 2;
    }
  }
  return buffer;
}
...
export function useAudioEngine(): AudioEngineRef {
  ...
  useEffect(() => {
    return () => {
      // biome-ignore lint/complexity/noForEach: existing hook logic
  ...
  const playSound = useCallback(
    (params: SynthParams, tuning: Partial<AudioTuningParams> = {}) => {
      ...
      lpFilter.frequency.value = Math.min(
        merged.lowpassFreq,
        ctx.sampleRate / 2 - 1,
      );
      ...
      if (params.noiseType) {
        ...
        masterGain.gain.linearRampToValueAtTime(
          (merged.volume / 100) * (params.gainStart ?? 0.3),
          now + attack,
        );
        if (decay > 0) {
          masterGain.gain.linearRampToValueAtTime(
            (merged.volume / 100) * (params.gainStart ?? 0.3) * sustain,
            now + attack + decay,
          );
        }
        ...
        masterGain.gain.setValueAtTime(0, now);
        masterGain.gain.linearRampToValueAtTime(
          (merged.volume / 100) * (params.gainStart ?? 0.5),
          now + attack,
        );
        if (decay > 0) {
          masterGain.gain.linearRampToValueAtTime(
            (merged.volume / 100) * (params.gainStart ?? 0.5) * sustain,
            now + attack + decay,
          );
        }
        masterGain.gain.setValueAtTime(
