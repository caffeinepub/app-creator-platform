...
import type { AudioTuningParams } from "../data/audioPresets";
import { getSoundById } from "../data/soundLibrary";
import { useAudioEngine } from "../hooks/useAudioEngine";
import { useKeyboardShortcuts } from "../hooks/useKeyboardShortcuts";
import type { PadConfig } from "../hooks/usePresetStorage";
...
export default function GesturePadGrid({
  pads,
  onPadsChange,
  tuning,
  editMode,
  ...
  for (const pad of pads) {
    if (pad.shortcutKey) shortcutMap[pad.shortcutKey] = () => triggerPad(pad);
  }
  useKeyboardShortcuts(shortcutMap, !editMode);

  const handlePadClick = (pad: PadConfig) => {
    if (editMode) setConfigPad(pad);
    else triggerPad(pad);
  };
          ...
          return (
            <button
              ...
              style={{
                ...
                boxShadow: isActive
                  ? `0 0 20px ${pad.color}60, 0 0 40px ${pad.color}30`
                  : "none",
              }}
            >
              {isActive && (
