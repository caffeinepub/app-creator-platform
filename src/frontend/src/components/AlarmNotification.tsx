...
export default function AlarmNotification({
  ...
  const { isPlaying, audioReady, playAlarm, stopAlarm, resumeAudioContext } =
    useAlarmSound();
  ...
  useEffect(() => {
    try {
      const result = playAlarmRef.current();
      if (result.needsUserGesture) setNeedsGesture(true);
    } catch (e) {
  ...
  useEffect(() => {
    if (audioReady && needsGesture) {
      try {
        ...
        if (!result.needsUserGesture) setNeedsGesture(false);
      } catch (e) {
