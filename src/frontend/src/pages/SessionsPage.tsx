...
function SessionCard({
  ...
  const typeConfig =
    projectTypeConfig[session.projectType] || projectTypeConfig.fullstack;
  const messageCount = session.messages.length;

  return (
    <button
      type="button"
      className="glass-card rounded-2xl border border-white/10 hover:border-brand/20 transition-all duration-300 group cursor-pointer overflow-hidden text-left w-full"
      onClick={onOpen}
    >
